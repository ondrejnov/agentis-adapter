...
- `context.adapter.runtime` rozhoduje, kudy run jde. `workflow` i `local` běží přes **workflow runtime** (`AgentJsonRpcService._is_workflow_runtime`, `_WORKFLOW_RUNTIMES`) — tj. kroky workflow YAML včetně commitu a PR; `local` navíc vynutí lokální executor (`WorkflowManager._resolve_executor`: runtime `local` přebije `workflow.executor` i `WORKFLOW_EXECUTOR`). Pojmenované workflow (followup `merge`/`close`) jde přes workflow runtime vždy. Ostatní/nevyplněný runtime padá do CLI session path (`BaseSessionManager`), která commit/PR nedělá. Environment `kubernetes` jako runtime hodnota byl odstraněn.
