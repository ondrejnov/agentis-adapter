...
_CLAUDE_SESSION_START_TIMEOUT_SEC = 15.0
...
class ClaudeSessionManager:
    ...
    ``_build_client``) so other single-prompt CLI agents (e.g. OpenCode run)
    can reuse the same lifecycle by subclassing.
    """

    # Prefix used for snapshot keys / adapter event kinds and the label shown
    # in source-snapshot records.
    _AGENT_LABEL = "claude"
    ...
    def start(
        ...
        if not sess.ready_event.wait(timeout=_CLAUDE_SESSION_START_TIMEOUT_SEC):
            self.abort(pending_key)
            with self._lock:
                self._sessions.pop(pending_key, None)
            raise RuntimeError(f"Claude session_id was not received within {_CLAUDE_SESSION_START_TIMEOUT_SEC:.0f}s")

        if not sess.session_id:
            with self._lock:
                self._sessions.pop(pending_key, None)
            raise RuntimeError(sess.start_error or "Claude session ended before reporting session_id")
    ...
    def send(
        ...
        """Pošle do existující session další prompt (nový claude run s `--resume`)."""
        with self._lock:
            sess = self._sessions.get(session_id)
        if sess is None:
            raise RuntimeError(f"Unknown claude session {session_id!r}")
        ...
        mapper = self._make_mapper(
            ...
            mode=mode,
            cwd=worktree,
            session_id_hint=sess.claude_session_id,
        )
        self._spawn_thread(sess, prompt=prompt, mapper=mapper, resume_id=sess.claude_session_id)
    ...
    def abort(self, session_id: str) -> None:
        ...
        if kubectl_target is not None:
            ...
            # přeruší stream, ale claude proces uvnitř podu běží dál.
            # Zabijeme tedy claude přímo v podu pomocí samostatného
            # `kubectl exec ... -- pkill`.
            if proc is not None:
                try:
                    proc.kill()
                    ...
                    sys.stderr.write(f"[claude-session] kill kubectl client failed for {session_id}: {exc}\n")
            self._remote_pkill_claude(kubectl_target, session_id)
            return
        ...
        # aby šly s claude CLI dolů i jeho potomci.
        try:
            pgid = os.getpgid(proc.pid)
        except ProcessLookupError:
            return
        except OSError as exc:
            sys.stderr.write(f"[claude-session] getpgid failed for {session_id}: {exc}\n")
            pgid = None
        ...
        if pgid is not None:
            try:
                ...
                sys.stderr.write(f"[claude-session] killpg failed for {session_id}: {exc}\n")
        ...
        try:
            ...
            sys.stderr.write(f"[claude-session] kill failed for {session_id}: {exc}\n")

    def _remote_pkill_claude(self, target: KubectlExecTarget, session_id: str) -> None:
        kubectl_path = target.kubectl
        if shutil.which(kubectl_path) is None and not os.path.isabs(kubectl_path):
            sys.stderr.write(
                f"[claude-session] remote pkill skipped for {session_id}: kubectl not on PATH ({kubectl_path})\n"
            )
            return
        ...
        try:
            ...
            sys.stderr.write(f"[claude-session] remote pkill timed out for {session_id}\n")
        except Exception as exc:  # noqa: BLE001
            sys.stderr.write(f"[claude-session] remote pkill failed for {session_id}: {exc}\n")

    def remove(self, session_id: str) -> None:
        with self._lock:
            self._sessions.pop(session_id, None)

    def _bind_session_id(self, sess: _ClaudeSession, session_id: str) -> None:
    ...
    def _emit_session_created(self, sess: _ClaudeSession, session_id: str) -> None:
        self._agentis_call(
    ...
    # ------------------------------------------------------------------
    # Agent-specific hooks (overridden by other single-prompt CLI agents)
    # ------------------------------------------------------------------
    ...
    def _build_client(self, sess: _ClaudeSession, resume_id: Optional[str]) -> ClaudeCodeClient:
    ...
    def _spawn_thread(
        ...
        *,
        ...
        thread = threading.Thread(
            ...
            name=f"claude-session-{thread_session_id}",
            daemon=True,
        )
        sess.thread = thread
        thread.start()
    ...
    async def _stream(
        ...
        #     kind="start_session",
        #     status="success",
        #     message="Spouštím Claude Code CLI.",
        #     event_id=run_event_id,
        #     data={"session_id": sess.session_id, "resume": bool(resume_id)},
        # )
        # self._emit_adapter_event(
        #     sess.context,
        #     kind="claude_run",
        ...
        try:
            ...
            if not sess.ready_event.is_set():
                sess.start_error = sess.start_error or "Claude session ended before reporting session_id"
                sess.ready_event.set()
    ...
    def _ensure_pull_request(
        ...
        try:
            ...
            sys.stderr.write(f"[claude-session] ensure_pull_request failed: {exc}\n")
            return None
        except Exception as exc:  # noqa: BLE001
            sys.stderr.write(f"[claude-session] ensure_pull_request unexpected error: {exc}\n")
            return None
    ...
    def _start_dev_server(self, sess: _ClaudeSession) -> dict[str, Any]:
    ...
    def _finish_session_actions(self, sess: _ClaudeSession, session_ref: str) -> list[dict[str, Any]]:
    ...
    def _agentis_call(self, method: str, params: dict[str, Any]) -> None:
        ...
        try:
            with AgentisJsonRpcClient(endpoint=endpoint, token=self.settings.agentis_token) as client:
                client.call(method=method, params=params, request_id=f"claude-{method}-{uuid4().hex}")
        except AgentisJsonRpcError as exc:
            sys.stderr.write(f"[claude-session] agentis {method} failed: {exc}\n")
        except Exception as exc:  # noqa: BLE001
            sys.stderr.write(f"[claude-session] agentis {method} unexpected error: {exc!r}\n")
