...
def make_client(service: AgentJsonRpcService | None = None) -> TestClient:
    ...
    if service is not None:
        ...
        app.state.internal_rpc_service = InternalRpcService(
...
def test_extract_json_from_plain_response_text():
    assert InternalRpcService._extract_json_from_text('{"items": ["a"]}') == {"items": ["a"]}


def test_extract_json_from_markdown_response_text():
    body = """Tady je výsledek:
...
"""
    ...
    assert InternalRpcService._extract_json_from_text(body) == {"items": [{"name": "Agentis"}]}
...
def test_knowledge_extractor_updates_related_source_task_knowledge(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    monkeypatch.setattr("common.rpc.internal.AgentisJsonRpcClient", fake_agentis_client_factory(captured_calls))

    service = InternalRpcService(settings=make_settings())
    service.session_registry.register(
        "sess-knowledge",
        AgentExecutionContextPayload(
            run_id="run-knowledge",
            task_id="019dc3cd-3bcb-7000-8000-000000000001",
            title="Extract knowledge",
            adapter=AdapterOptionsPayload(agent="knowledge-extractor", model="gpt-5.4"),
        ),
    )

    result = service.session_idle(
        SessionIdleParams(
    ...
    knowledge_calls = [call for call in captured_calls if call["method"] == "task.update_related_knowledge"]
    assert len(knowledge_calls) == 1
    assert knowledge_calls[0]["request_id"] == "internal-update-related-knowledge-019dc3cd-3bcb-7000-8000-000000000001"
    assert knowledge_calls[0]["params"] == {
        "id": "019dc3cd-3bcb-7000-8000-000000000001",
        "knowledge": {"items": [{"name": "Agentis"}]},
    }
...
def test_session_idle_uses_kubernetes_namespace_in_dev_server_attachment(monkeypatch):
    captured_calls: list[dict[str, Any]] = []

    def fake_forward(self, request_id: str, method: str, params: dict[str, Any]) -> dict[str, Any]:
        captured_calls.append({"method": method, "params": params})
        return {"ok": True, "method": method}

    monkeypatch.setattr(InternalRpcService, "_forward", fake_forward)
    monkeypatch.setattr(
        InternalRpcService,
        ...
        lambda self, context: {
            ...
            "working_dir": "/var/www/worktrees/019dc3cd-3bcb",
        },
    )
    monkeypatch.setattr(InternalRpcService, "_ensure_pull_request", lambda self, context: None)
    monkeypatch.setattr(
        InternalRpcService,
        "_start_dev_server",
        lambda self, context: {
            "namespace": "task-17-implementace-nove-fu",
            "pod_name": "opencode-abc123",
            "working_dir": "/var/www/worktrees/019dc3cd-3bcb",
        },
    )

    context_payload = make_start_params()["context"] | {
        "task_id": "019dc3cd-3bcb",
        "task_number": 17,
        "title": "Implementace nove funkce",
    ...
    service = InternalRpcService(settings=make_settings(namespace_prefix="Task"))
    service.session_registry.register("sess-1", AgentExecutionContextPayload(**context_payload))

    response = service.session_idle(
        SessionIdleParams(
            session_id="sess-1",
            messages=[
                {
                    "info": {"id": "msg-1", "role": "assistant"},
                    "parts": [{"type": "text", "text": "Hotovo."}],
                }
            ],
        )
    )
    ...
    assert comment_calls[0]["params"]["attachments"][-1] == {
        "label": "Dev server",
        "value": "http://app-task-17-implementace-nove-fu.dev.agentis.cz",
        "type": "url",
    }


def test_project_scope_completion_actions_do_not_offer_task_branch_cleanup():
    context = AgentExecutionContextPayload(
        run_id="run-1",
        task_id="task-1",
        title="Implementace nove funkce",
    ...
    assert InternalRpcService._completion_actions(context) == []


def test_session_idle_uses_adapter_task_status(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr(InternalRpcService, "_forward", fake_forward)
    monkeypatch.setattr(InternalRpcService, "_commit_session_changes", lambda self, context: None)
    monkeypatch.setattr(InternalRpcService, "_ensure_pull_request", lambda self, context: None)
    monkeypatch.setattr(InternalRpcService, "_start_dev_server", lambda self, context: None)

    context_payload = make_start_params()["context"] | {
        "project_github_repo": None,
        "adapter": {"task_status": TaskStatus.BLOCKED},
    }
    service = InternalRpcService(settings=make_settings())
    service.session_registry.register("sess-1", AgentExecutionContextPayload(**context_payload))
    ...
    response = service.session_idle(
        SessionIdleParams(
            session_id="sess-1",
            messages=[{"info": {"id": "msg-1", "role": "assistant"}, "parts": [{"type": "text", "text": "Hotovo."}]}],
        )
    )

    assert response == {"ok": True}
    comment_calls = [call for call in captured_calls if call["method"] == "task.add_agent_comment"]
    assert comment_calls[0]["params"]["status"] == TaskStatus.BLOCKED


def test_session_idle_project_scope_skips_commit_pr_and_dev_server(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr(
        InternalRpcService,
        "_commit_session_changes",
        lambda self, context: (_ for _ in ()).throw(AssertionError("commit must be skipped for project scope")),
    )
    monkeypatch.setattr(
        InternalRpcService,
        "_ensure_pull_request",
        lambda self, context: (_ for _ in ()).throw(AssertionError("PR must be skipped for project scope")),
    )
    monkeypatch.setattr(
        InternalRpcService,
        "_start_dev_server",
        lambda self, context: (_ for _ in ()).throw(AssertionError("dev server must be skipped for project scope")),
    )

    context_payload = make_start_params()["context"] | {
        "project_github_repo": "example/repo",
        "adapter": {"scope": "project", "agent": "build", "model": "gpt-5.4"},
    }
    ...
    adapter_event_calls = [call for call in captured_calls if call["method"] == "run.adapter_event"]
    assert len(adapter_event_calls) == 1
    assert strip_event_id(adapter_event_calls[0]["params"]) == {
        "run_id": "run-1",
        "kind": "idle",
        "status": "success",
...
def test_session_idle_sends_screenshot_images(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr(InternalRpcService, "_start_dev_server", lambda self, context: None)
    monkeypatch.setattr(
        "common.rpc.internal.collect_screenshot_images",
        lambda project_root: [{"name": "task.png", "content": "cG5n"}],
    )

    context_payload = make_start_params()["context"] | {"project_github_repo": None}
    service = InternalRpcService(settings=make_settings(namespace_prefix="Task"))
    service.session_registry.register("sess-1", AgentExecutionContextPayload(**context_payload))
    ...
    comment_calls = [call for call in captured_calls if call["method"] == "task.add_agent_comment"]
    assert len(comment_calls) == 1
    assert comment_calls[0]["params"]["images"] == [{"name": "task.png", "content": "cG5n"}]


def test_session_idle_writes_changes_diff_attachment(monkeypatch, tmp_path: Path):
    captured_calls: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr(InternalRpcService, "_forward", fake_forward)
    monkeypatch.setattr(
        "common.rpc.internal.write_changes_diff_best_effort",
        lambda worktree, snapshot_key, label: diff_calls.append((worktree, snapshot_key, label))
        or type(
            "Result",
            (),
            {
    ...
    service = InternalRpcService(settings=make_settings(worktree_root=Path("/var/www/worktrees")))
    context = AgentExecutionContextPayload(**(make_start_params()["context"] | {"project_github_repo": None}))
    service.session_registry.register("sess-1", context)
    service.session_registry.set_snapshot_key("sess-1", "snap-feedback")
    ...
    assert response == {"ok": True}
    assert diff_calls == [(Path("/var/www/worktrees/task-1"), "snap-feedback", "opencode-idle")]
    comment_calls = [call for call in captured_calls if call["method"] == "task.add_agent_comment"]
    assert comment_calls[0]["params"]["attachments"] == [{"label": "Changes diff", "value": diff_content, "type": "diff"}]
...
def test_jsonrpc_internal_route_uses_shared_dispatcher():
    class FakeInternalRpcService:
        def session_idle(self, params):
            assert params.session_id == "sess-1"
            return {"ok": True}

    client = make_client()
    client.app.state.internal_rpc_service = FakeInternalRpcService()
    ...
    response = client.post(
        ...
        json={
            "jsonrpc": "2.0",
            "id": "idle-1",
            "method": "session.session_idle",
            "params": {"session_id": "sess-1", "messages": []},
        },
    )

    assert response.status_code == 200
    assert response.json() == {"jsonrpc": "2.0", "id": "idle-1", "result": {"ok": True}}
...
def test_local_runtime_without_local_deploy_manifest_starts_local_opencode(monkeypatch, tmp_path):
    captured: dict[str, Any] = {}
    workspace = tmp_path / "worktree"
    agentis_dir = workspace / ".agentis"
    agentis_dir.mkdir(parents=True)
    ...
    class FakeStdout:
        def __init__(self) -> None:
            self.lines = ["listening on http://0.0.0.0:54321\n"]
    ...
    assert ". .agentis/local-setup.sh" in captured["command"][2]
    assert "exec opencode web --hostname 0.0.0.0" in captured["command"][2]
    assert captured["cwd"] == str(workspace)
    assert captured["env"]["AGENTIS_URL"] == "http://adapter.internal:8001"
    assert service.wait_ready(timeout=0.0)["url"] == "http://127.0.0.1:54321"


def test_local_runtime_close_stops_process_without_kubectl_delete(monkeypatch, tmp_path):
    calls: list[str] = []
    ...
    service = KubernetesAdapterService(context, make_settings(manifest_path=Path("/tmp/manifests")))
    result = service.close()

    assert result["manifest_path"] is None
    assert result["local_process_stopped"] is True
    assert result["worktree_removed"] is True
    assert result["branch_deleted"] is True
    assert "terminate" in calls
    assert "kubectl_delete" not in calls
...
def test_api_internal_dispatches_known_method():
    client = make_client()
    response = client.post(
        "/api-internal",
        json={
            "jsonrpc": "2.0",
            "id": 1,
            "method": "session.start_task",
            "params": {"id": "run-1", "session_id": "sess-1"},
        },
    )
    ...
    assert response.json() == {
        "jsonrpc": "2.0",
        "id": 1,
        "result": {"ok": True, "method": "session.start_task", "forwarded": False},
    }


def test_api_internal_forwards_task_add_question(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
        ...
        json={
            ...
            "params": {
                ...
                "questions": [
                    {
                        "header": "Superpower",
                        "question": "Jakou superpower bys chtel/a mit?",
                        "options": [{"label": "Letani", "description": "Pohyb vzduchem bez omezeni."}],
                    }
                ],
    ...
    assert response.json() == {
        "jsonrpc": "2.0",
        "id": 7,
        "result": {"ok": True, "method": "task.add_question"},
    }
    assert len(captured_calls) == 1
    assert captured_calls[0]["endpoint"] == "http://10.0.0.205:8891"
    assert captured_calls[0]["timeout"] == 10.0
    assert captured_calls[0]["token"] == "1234"
    assert captured_calls[0]["request_id"] == "internal-add-question-sess-1-question-1"
    assert captured_calls[0]["method"] == "task.add_question"
...
def test_api_internal_forwards_session_error(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    ...
    assert response.json() == {
        "jsonrpc": "2.0",
        "id": 8,
        "result": {"ok": True, "method": "session.session_error"},
    }
    assert len(captured_calls) == 1
    assert captured_calls[0]["endpoint"] == "http://10.0.0.205:8891"
    assert captured_calls[0]["timeout"] == 10.0
    assert captured_calls[0]["token"] == "1234"
    assert captured_calls[0]["request_id"] == "internal-session-error-sess-1"
    assert captured_calls[0]["method"] == "session.session_error"
    assert captured_calls[0]["params"] == {
        "type": "session.error",
        "properties": {
            "sessionID": "sess-1",
...
def test_api_internal_forwards_start_task_and_session_events(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr(
        ...
        lambda self, context: {
            "status": "skipped",
            "reason": "clean_worktree",
            "working_dir": "/var/www/worktrees/task-1",
        },
    )
    monkeypatch.setattr(
        InternalRpcService,
    ...
    start_task_response = client.post(
        "/api-internal",
        json={
            "jsonrpc": "2.0",
            "id": 2,
    ...
    session_created_response = client.post(
        "/api-internal",
        json={
            "jsonrpc": "2.0",
            "id": 3,
            "method": "session.session_created",
            "params": {"session": {"id": "sess-1", "parentID": None, "title": "OpenCode"}},
        },
    )
    session_update_response = client.post(
        "/api-internal",
        json={
            "jsonrpc": "2.0",
            "id": 4,
            "method": "session.session_update",
            "params": {"session_id": "sess-1", "session": {"title": "Updated"}},
        },
    )
    session_idle_response = client.post(
        "/api-internal",
        json={
            "jsonrpc": "2.0",
            "id": 5,
                ...
                "messages": [
                    {
                        "info": {"id": "msg-1", "role": "assistant"},
                        "parts": [
                            {"type": "step-start", "id": "part-1"},
                            {"type": "text", "text": "Hotovo, upravil jsem README."},
    ...
    activity_log_response = client.post(
        "/api-internal",
        json={
            "jsonrpc": "2.0",
            "id": 6,
    ...
    assert session_created_response.status_code == 200
    assert session_update_response.status_code == 200
    assert session_idle_response.status_code == 200
    assert activity_log_response.status_code == 200

    assert start_task_response.json() == {
        "jsonrpc": "2.0",
        "id": 2,
        "result": {"ok": True, "method": "session.start_task", "forwarded": False},
    }
    assert [call["method"] for call in captured_calls] == [
    ...
    assert all(call["endpoint"] == "http://10.0.0.205:8891" for call in captured_calls)
    assert all(call["token"] == "1234" for call in captured_calls)
    assert captured_calls[0]["params"] == {"session": {"id": "sess-1", "parentID": None, "title": "OpenCode"}}
    ...
    assert captured_calls[3]["params"] == {
        "session_id": "sess-1",
        "body": "Hotovo, upravil jsem README.",
        "attachments": [],
        "images": [],
        "artifacts": [],
        "status": TaskStatus.IN_REVIEW,
        "comment_type": "primary",
        "actions": expected_completion_actions(),
    }
    assert strip_event_id(captured_calls[4]["params"]) == {
        "run_id": "run-1",
        "kind": "idle",
        "status": "success",
        "message": "OpenCode session je neaktivní.",
    ...
    assert captured_calls[5]["params"] == {
        "session_id": "sess-1",
        "messages": [{"info": {"id": "msg-1", "role": "assistant"}, "parts": []}],
    }


def test_session_idle_commits_dirty_worktree(monkeypatch, tmp_path):
    captured_calls: list[dict[str, Any]] = []
    git_calls: list[tuple[Path, tuple[str, ...]]] = []
    ...
    def fake_run_git(cwd: Path, *args: str) -> str:
        ...
        if args == ("add", "--all"):
            return ""
        if "commit" in args:
            return "[task-1 abc123] chore: checkpoint task-1"
        if args == ("rev-parse", "HEAD"):
            return "abc123"
        raise AssertionError(f"Unexpected git command: {args}")

    settings = make_settings(worktree_root=tmp_path / "worktrees")
    worktree_path = settings.worktree_root / "task-1"
    worktree_path.mkdir(parents=True)

    monkeypatch.setattr("common.rpc.internal.AgentisJsonRpcClient", fake_agentis_client_factory(captured_calls))
    monkeypatch.setattr(KubernetesAdapterService, "_git_succeeds", staticmethod(lambda cwd, *args: True))
    monkeypatch.setattr(KubernetesAdapterService, "_run_git", staticmethod(fake_run_git))
    monkeypatch.setattr(InternalRpcService, "_ensure_pull_request", lambda self, context: None)
    monkeypatch.setattr(
        InternalRpcService,
    ...
    service = InternalRpcService(settings=settings)
    context = AgentExecutionContextPayload(**(make_start_params()["context"] | {"project_github_repo": "example/repo"}))
    service.session_registry.register("sess-1", context)
    ...
    assert git_calls == [
        ...
        (
            ...
            (
                ...
                "commit",
                "-m",
                "TASK: #None - Implementace nove funkce",
            ),
        ),
    ...
    assert strip_event_id(adapter_event_calls[0]["params"]) == {
        ...
        "kind": "commit",
        "status": "success",
        "message": "Rozpracovaný kód byl commitnut.",
        "data": {
            "session_id": "sess-1",
            "status": "success",
            "working_dir": str(worktree_path),
            "commit_sha": "abc123",
            "commit_message": "TASK: #None - Implementace nove funkce",
        },
    }


def test_session_idle_replaces_ide_workdir_placeholder_with_worktree_path(monkeypatch, tmp_path):
    captured_calls: list[dict[str, Any]] = []

    settings = make_settings(worktree_root=tmp_path / "worktrees")
    expected_worktree_path = settings.worktree_root / "task-1"
    ...
    monkeypatch.setattr(
        ...
        lambda self, context: {
            "status": "skipped",
            "reason": "clean_worktree",
            "working_dir": str(expected_worktree_path),
        },
    )
    monkeypatch.setattr(
        InternalRpcService,
    ...
    comment_calls = [call for call in captured_calls if call["method"] == "task.add_agent_comment"]
    assert len(comment_calls) == 1
    assert comment_calls[0]["params"]["attachments"][0] == {
        "label": "Directory",
        "value": f"vscode://file/{expected_worktree_path}?windowId=_blank",
        "type": "url",
    }


def test_session_idle_emits_idle_event_when_project_repo_is_present(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    ...
    class FakePrService:
        instances: list["FakePrService"] = []

        def __init__(self, context: Any, worktree_path: Any, branch: str) -> None:
            self.context = context
            self.worktree_path = worktree_path
            self.branch = branch
            FakePrService.instances.append(self)

        def ensure_pull_request_result(self) -> GithubPrResult:
            return GithubPrResult(url="https://github.com/example/repo/pull/42", created=True)

    monkeypatch.setattr("common.rpc.internal.AgentisJsonRpcClient", fake_agentis_client_factory(captured_calls))
    monkeypatch.setattr("common.rpc.internal.GithubPrService", FakePrService)
    monkeypatch.setattr(
        InternalRpcService,
        "_start_dev_server",
        lambda self, context: {
    ...
    start_params = make_start_params()
    start_params["context"]["project_github_repo"] = "example/repo"
    start_response = client.post(
        "/api",
        json={"jsonrpc": "2.0", "id": 1, "method": "start", "params": start_params},
    )
    assert start_response.status_code == 200
    ...
    adapter_event_calls = [call for call in captured_calls if call["method"] == "run.adapter_event"]
    assert len(adapter_event_calls) == 4
    assert strip_event_id(adapter_event_calls[0]["params"]) == {
        "run_id": "run-1",
        "kind": "commit",
        "status": "success",
        "message": "Žádné změny ke commitnutí.",
        "data": {
            "session_id": "sess-1",
            "status": "skipped",
            "reason": "clean_worktree",
    ...
    assert strip_event_id(adapter_event_calls[1]["params"]) == {
        "run_id": "run-1",
        "kind": "dev_server",
        "status": "started",
    ...
    assert strip_event_id(adapter_event_calls[2]["params"]) == {
        "run_id": "run-1",
        "kind": "dev_server",
        "status": "success",
    ...
    assert strip_event_id(adapter_event_calls[3]["params"]) == {
        "run_id": "run-1",
        "kind": "idle",
        "status": "success",
        "message": "OpenCode session je neaktivní.",
...
def test_session_idle_skips_pr_when_repo_missing(monkeypatch):
    captured_calls: list[dict[str, Any]] = []
    ...
    def _fail(*_args: Any, **_kwargs: Any) -> None:
        raise AssertionError("GithubPrService must not be instantiated when project_github_repo is missing")

    monkeypatch.setattr("common.rpc.internal.AgentisJsonRpcClient", fake_agentis_client_factory(captured_calls))
    monkeypatch.setattr("common.rpc.internal.GithubPrService", _fail)
    monkeypatch.setattr(
        InternalRpcService,
        "_start_dev_server",
        lambda self, context: (_ for _ in ()).throw(RuntimeError("dev server unavailable")),
    )
    monkeypatch.setattr(
        InternalRpcService,
        "_commit_session_changes",
