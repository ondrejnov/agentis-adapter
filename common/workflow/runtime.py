...
def build_bash_wrapper(script: str | None, *, workdir: str | None = None, workdir_env: str = "WORKDIR") -> str:
    """Obalí krok do bashe; `workdir` (workingDir kroku/workflow) má přednost před `$WORKDIR`."""

    if script is None:
        raise ValueError("Workflow step has no run script")
...
def build_job_manifest(
    ...
    *,
    ...
    container: dict[str, Any] = {
        "name": "step",
        "image": image,
        "command": ["/bin/bash", "-lc", build_bash_wrapper(step.run, workdir=working_dir)],
